...
3. For EACH topic in topics.yaml, use `curl` via the Bash tool (network works here):
   a. PubMed E-utilities, esearch then esummary:
      curl -sS "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmode=json&retmax=40&datetype=edat&reldate=<lookback_days>&term=<url-encoded pubmed query>&tool=sunscreen-pipeline&email=steven.w@stevenwangmd.com"
      ...
      Sleep 0.4s between NCBI calls (no API key = 3 req/s cap).
   b. OpenAlex:
